...
import { buildQuery } from 'src/general/helpers';
...
export class ItemsService {
  ...
  async findAll(
    ...
    const { query, sort } = buildQuery(name, rate, order, minPrice, maxPrice);

    return this.itemModel.find(query).sort(sort).exec();
  }

  // async createRandomItems(): Promise<Item[]> {
  //   const randomItems = [];

  //   for (let i = 0; i < 10; i++) {
  //     const randomName = 'Item' + i; // Генерувати імена типу "Item0", "Item1", і т. д.
  //     const newItem = new this.itemModel({
  //       name: randomName,
  //       description: 'Random Description ' + i,
  //       rate: Math.floor(Math.random() * 6),
  //       price: Math.floor(Math.random() * 100),
  //       image: 'random-image-' + i + '.jpg',
  //     });
  //     randomItems.push(await newItem.save());
  //   }
